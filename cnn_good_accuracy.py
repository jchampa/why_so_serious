...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.model.add(tf.keras.layers.Conv2D(filters=64, kernel_size=(3,3), strides=(2,2),input_shape=[244, 244, 1], padding='SAME'))
        ...
        #conv2
        self.model.add(tf.keras.layers.Conv2D(filters=128, kernel_size=(3,3), strides=(2,2), padding='SAME'))
        self.model.add(tf.keras.layers.BatchNormalization(epsilon = self.epsilon))
        self.model.add(tf.keras.layers.Activation('relu'))
        self.model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))

        #conv3
        self.model.add(tf.keras.layers.Conv2D(filters=256, kernel_size=(3,3), strides=(2,2), padding='SAME'))
        self.model.add(tf.keras.layers.BatchNormalization(epsilon = self.epsilon))
        self.model.add(tf.keras.layers.Activation('relu'))
        # self.model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
...
def train(model, train_inputs, train_labels):
    ...
    for i in range(0, len(train_inputs)-model.batch_size, model.batch_size):
        ...
        tf.reshape(batchInputs, [model.batch_size, 244, 244, 1])
        # if len(tf.shape(batchInputs)) == 5:
        #     tf.squeeze(batchInputs, [4])
...
def main():
    ...
    model.add(tf.keras.layers.Conv2D(filters=64, kernel_size=(3,3), strides=(2,2),input_shape=[244, 244, 1], padding='SAME'))
