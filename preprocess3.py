...
def get_data(images_path, labels_path):
			...
			with open(file_path, 'r') as f:
				...
				if key in data_dictionary:
					for k in data_dictionary[key]:
						#first three images have neutral label
	...
	for key in data_dictionary:
		if(len(data_dictionary[key].keys())<6):
			# print("KEY BAD")
			# print(key)
			for j in range(1, 5):
				if j == 3:
	...
	np.save('inputs3.npy', inputs, allow_pickle=True)
	np.save('labels3.npy', labels, allow_pickle=True)
